...
namespace capa_datos
{
    public class cd_cliente
    {
        ...
        public ob_cliente buscar_cliente(string clie)
        {
            ...
            comando.Connection = conexion.abrir_conexion();
            comando.CommandText = "mostrar_cliente_x_ruc";
            comando.CommandType = CommandType.StoredProcedure;
            ...
            if (tabla.Rows[0] != null)
            {
                ...
                obcli.cli_id = Convert.ToInt32(clien["cli_id"].ToString());
                obcli.cli_ruc = clien["cli_ruc"].ToString();
                obcli.cli_nom = clien["cli_nom"].ToString();
                obcli.cli_ape = clien["cli_ape"].ToString();
                obcli.cli_dire = clien["cli_dire"].ToString();
                obcli.cli_fnaci = clien["cli_fnaci"].ToString();
                obcli.cli_tel = clien["cli_tel"].ToString();
                obcli.cli_corr = clien["cli_corr"].ToString();
                obcli.cli_est = clien["cli_est"].ToString();
                comando.Parameters.Clear();
                conexion.cerrar_conexion();
                return obcli;
            }
        ...
        public void nuevo_cliente(string cli_ruc, string cli_nom, string cli_ape, string cli_dire, string cli_tel, string cli_corr, string cli_fnaci)
        {
            comando.Connection = conexion.abrir_conexion();
            comando.CommandText = "crear_editar_cliente";
            comando.CommandType = CommandType.StoredProcedure;
